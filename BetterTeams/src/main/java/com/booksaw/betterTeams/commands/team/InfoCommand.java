...
public class InfoCommand extends SubCommand {
	...
	public static List<@Nullable String> getInfoMessages(Team team, CommandSender viewer) {
		...
		TeamPlayer viewerTeamPlayer = (viewer instanceof Player) ? team.getTeamPlayer((Player) viewer) : null;
		boolean isAdmin = viewer.hasPermission("betterTeams.admin");
		boolean isMember = viewerTeamPlayer != null;

		// Bank balance is private - only visible to team members and server admins
		if (isMember || isAdmin) {
		...
		// Member count is only shown to server admins and team management (owner/co-owner) - normal members do not see it
		boolean canSeeMemberCount = isAdmin || (isMember
				&& (viewerTeamPlayer.getRank() == PlayerRank.OWNER || viewerTeamPlayer.getRank() == PlayerRank.CO_OWNER));

		if (canSeeMemberCount) {
			infoMessages.add(MessageManager.getMessage("info.limits",
					currentMembers, maxMembers, currentWarps, maxWarps));
		} else {
			infoMessages.add(MessageManager.getMessage("info.limitsNoMembers", currentWarps, maxWarps));
		}
	...
	private void displayTeamInfo(CommandSender sender, Team team) {
		List<String> toDisplay = getInfoMessages(team, sender);
