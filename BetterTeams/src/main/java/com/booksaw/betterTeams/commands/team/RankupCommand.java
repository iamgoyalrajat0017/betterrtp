...
public class RankupCommand extends TeamSubCommand {
	...
	public CommandResponse onCommand(TeamPlayer player, String label, String[] args, Team team) {
		...
		String signature = "rankup:" + nextLevel.getLevel() + ":" + price;
		if (!ConfirmationManager.isConfirmed(player.getPlayerUUID(), signature)) {
			...
			String formattedPrice = score ? String.valueOf(price) : MoneyUtils.getFormattedDouble(price);
			return new CommandResponse(new ReferencedFormatMessage("rankup.confirm",
					nextLevel.getLevel(), formattedPrice, TimeUtil.formatDuration(expirySeconds * 1000L)));
		}

		// Re-validate everything at the point of confirmation, state may have changed since the request
		nextLevel = LevelManager.getNextLevel(team.getLevel());
		if (nextLevel == null || nextLevel.getLevel() != Integer.parseInt(signature.split(":")[1])) {
			// team's level changed in the meantime - the confirmed action is no longer valid
			return new CommandResponse("rankup.max");
		}
		if (score) {
			if (team.getScore() < price) {
